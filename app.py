...
# laoding the data
data_log = pd.read_csv('energy_consumption.csv')
...
ets_model = pickle.load(open('ETS_model.pkl', 'rb'))
svr_model = pickle.load(open('SVR_model.pkl', 'rb'))
lstm_model = pickle.load(open('LSTM_model.pkl', 'rb'))
hybrid_model = pickle.load(open('Hybrid_model.pkl', 'rb'))
...
def predict_arima(date):
    ...
    return prediction()
# Function to predict using SARIMA model
def predict_sarima(date):
    prediction_sarima = sarima_model.predict(date)
    return prediction()
...
# Function to predict using SVR model
def predict_svr(date):
    prediction_svr = svr_model.predict(date)
    return prediction() 
...
def predict_lstm(date):
    ...
    return prediction() 
# Function to predict using Hybrid model
def predict_hybrid(date):
    prediction_hybrid = hybrid_model.predict(date)
    return prediction() 
@app.route('/')
def index():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if selected_model == 'arima':
                ...
                historical_data = extract_historical_data(date)
                prediction = predict_sarima(historical_data)
            elif selected_model == 'ets':
                historical_data = extract_historical_data(date)
                prediction = predict_ets(historical_data)
            elif selected_model == 'svr':
                historical_data = extract_historical_data(date)
                prediction = predict_svr(historical_data)
                ...
                historical_data = extract_historical_data(date)
                prediction = predict_hybrid(historical_data)
            else:
                prediction = 'none'
